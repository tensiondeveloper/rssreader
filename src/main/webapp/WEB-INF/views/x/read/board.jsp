...
<div class="main col-md-9 col-mg-3 col-md-push-3"
	...
	<sec:authentication property="name" var="secName"/>
		...
		<sec:authorize access="${secName==contents.id}">
			<a href="/read/rewrite?board=${contents.board_type}&seq=${contents.board_seq}" type="submit" class="btn btn-dark"
				style="margin-right: 10px;">수정</a> 
				<a
				href="/read/delete/${contents.board_seq}?board=${contents.board_type}" type="submit"
			...
			<a
			href="/read/${contents.board_type}" type="submit"
		...
		<c:forEach var="reply" items="${reply}" varStatus="status">	
			...
			<sec:authorize access="${username==reply.id}">
				<form action="/read/reply_delete/${reply.reply_seq}" method="post" style="text-align: right;">
...
<li class="list-group-item note-form clearfix">
	<form action="/read/replyadd?${_csrf.parameterName}=${_csrf.token}"
