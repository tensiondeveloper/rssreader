...
function searchfn(){
	...
	location.href = "read?page=0&search="+$("#search").val()+"&keyword="+$("#keyword").val()+"&pangename="+pagename;
...
{
	location.href = "read?page="+page+"&search="+'${search}'+"&keyword="+'${keyword}'+"&pangename="+pagename;
}

function delete_read_board((seq){
					...
					<tbody>
						<c:forEach var="list" items="${list}" varStatus="status">				
							<tr>
								...
								<td><a href="read_rewrite?seq=${list.board_seq}">${list.board_title}</a></td>
								<td >${list.id}</td>
								<td class="thide">${list.board_date}</td>
								<td class="thide">${list.board_count}</td>
								<td class="thide"><button onclick="delete_read_board('${list.board_seq}')">삭제</button></td>
							</tr>
						</c:forEach>
					</tbody>	
